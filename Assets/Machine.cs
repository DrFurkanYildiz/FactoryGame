...
public class Machine : PlacedObject, IItemSlot
{
    ...
    {
        _recipeInputs = PlacedObjectTypeSo.itemRecipeSo.inputItemList;
        _recipeOutputs = PlacedObjectTypeSo.itemRecipeSo.outputItemList;

        _inputGates = PlacedObjectTypeSo.GetIOTypePositionList(IOType.Input, Origin, Dir);
        _outputGates = PlacedObjectTypeSo.GetIOTypePositionList(IOType.Output, Origin, Dir);
    ...
    private void Update()
    {
        if (HasEnoughItemsToCraft())
        {
            ...
            if (_craftingProgress >= PlacedObjectTypeSo.itemRecipeSo.craftingEffort)
            {
    ...
    private void OutputStorageAction()
    {
        ...
        foreach (var outputGate in _outputGates)
        {
            ...
            var outputPosition = outputGate + PlacedObjectTypeSo.GetDirForwardVector(Dir);
            var outputTile = Grid.GetGridObject(outputPosition);
            if (outputTile.OwnedObject is not IItemSlot itemSlotObj)
            {
                Debug.LogWarning($"Komşu nesne IItemSlot değil: {outputTile.OwnedObject}");
                continue;
            }
